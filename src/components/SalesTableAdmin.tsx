...
const SalesTableAdmin: React.FC<SalesTableAdminProps> = ({
  ...
  if (salesLoading) {
    return <div className="py-10 text-center text-gray-400">Loading sales...</div>;
  }
  if (salesError) {
    return (
  ...
  if (!sales.length) {
    return <div className="px-3 py-8 italic text-gray-400 text-center">No sales records.</div>;
  }
  return (
